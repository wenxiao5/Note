...
We want evaluate (1) $\int_X f$; (2) the probability of a subset $a\subset X :P(a)=\int_a p(x)dx$, where $h(x)=\mathbf{1}_{x\in a}$

The Monte Carlo approach is as follows: Given $X_1, X_2, \cdots, X_n$ drawn i.i.d from the pdf
$p$, estimate $\xi $ by the empirical average $$\hat{\xi}=\frac{1}{n}\sum_{i=1}^nh(X_i)$$

$\mathbb{E}_p[\hat{\xi}]=\mathbb{E}_p[h(X)]=\xi$. $\hat{\xi} \stackrel{a.s.}{\longrightarrow}\xi$ by SLLW.

$Var(\hat{\xi}-\xi)=\frac{1}{n}Var[h(x)]=O\left(\frac{1}{n}\right)$
