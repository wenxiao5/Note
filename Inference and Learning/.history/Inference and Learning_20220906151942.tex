...
\begin{equation}
    ...
    \nonumber
\end{equation}
which means the error of estimators $$\hat{w}-w^*=O(\frac{1}{\sqrt{n}})$$
...
Approach 2: (Penalized ERM) $$\min_\delta [R_{emp}(\delta)+J(\delta)]$$ ($J(\delta)$ is regularization (penalty) term)
...
We do not observe $h(x)$ directly, but we observe $Y\sim P_x$ with $\mathbb{E}[Y|X=x]=h(x)$ and $(Y|X=x)-h(x)$ is bounded.
\begin{example}
...
\begin{enumerate}[$\bullet$]
    \item Prick Sequence $\{a_n\}$ such that $\sum_n=1^\infty a_n=\infty$ and $\sum_n=1^\infty a^2_n<\infty$ (should converge to $0$ but not too quick)
    \item Initialize $X_1$
...
\begin{example}
    $Y_n=X_n+Z_n$ where $Z_n$ is independent of $Y_n$ with $\mathbb{E}[Z_n]=0$ and $Z_n$ is bounded. Then, $$X_{n+1}=X_n-a_n(X_n+Z_n)=(1-a_n)X_n-a_n Z_n$$
\end{example}
