...
\begin{enumerate}[(1)]
    ...
    \item \textbf{Binary:} $\text{binary}(x)=\text{step}(x)=\left\{\begin{matrix}
        ...
        0,&x<0
    \end{matrix}\right.$
    \item \textbf{Sigmoid:} $\sigma(x)=\frac{1}{e^{-x}}$, $\frac{d\sigma(x)}{dx}=\frac{1}{1+e^{-x}}\left(1-\frac{1}{1+e^{-x}}\right)=\sigma(x)\cdot(1-\sigma(x))$
    \item \textbf{TanH:} $g(x)=\frac{e^x-e^{-x}}{e^x+e^{-x}}$
...
\begin{enumerate}[$\bullet$]
    \item Initialize weights (including a bias term) to zero, e.g. $W=0^m$.
...
The most important difference between naive Bayes and logistic regression is that logistic regression is a \textbf{discriminative classifier} while naive Bayes is a \textbf{generative classifier}.

Suppose we want to classify class $A$ (dogs) and class $B$ (cats):
\begin{enumerate}
    \item \textbf{Generative model:} A generative model would have the goal of understanding what dogs look like and what cats look like. You might literally ask such a model to ‘generate’, i.e., draw, a dog.
    \item \textbf{Discriminative model:} A discriminative model, by contrast, is only trying to learn to distinguish the classes (perhaps without learning much about them).
\end{enumerate}
