...
(1) Bias $\frac{1}{k}\sum_{j=1}^k (\vec{Y})\hat{\theta}_n(\vec{Y}^{(j)})-\theta$; (2) Variance $\mathbb{E}[\hat{\theta}_n^2(\vec{Y})]-\mathbb{E}^2[\hat{\theta}_n(\vec{Y})]$; (3) CDF $G_{n}(t)=P(\hat{\theta}_n(\vec{Y})<t),\forall t$
