...
$\Rightarrow$ Compute pseudo-data $\vec{Y_i^*}^{(j)}=\hat{a}_n+\hat{b}_n\frac{i}{n}+\vec{E_i^*}^{(j)}$\\
$\Rightarrow$ Compute LS estimator $$(\hat{a}_n^{(j)},\hat{b}_n^{(j)})=\argmin_{(a,b)}\sum_{i=1}^n(\vec{Y_i^*}^{(j)}-a-b\frac{i}{n})^2$$
