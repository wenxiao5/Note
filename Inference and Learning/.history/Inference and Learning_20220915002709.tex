...
\begin{enumerate}
    ...
    \end{enumerate}
    \textbf{Computational cost:} The computational cost of GD is $O(dn)$ operations per iteration. Since GD has exponential convergence, the number of iterations needed to reach an optimization error of $\rho$ is $O(\log\frac{1}{\rho})$. Hence GD incurs a total compu-
    tational cost of O(dn log ρ1 ) to reach a solution wk such that
    \item[\textbf{SGD}]
    \begin{enumerate}[$\bullet$]
        \item Initialize $W_1$
        \item Update for $k\geq 1$,
        ...
        \end{enumerate}
    \end{enumerate}
    After $k$ iterations, $\mathbb{E}[f(W_k)]\leq \min_W f(W)+\varepsilon$, for $k=O(\frac{1}{\varepsilon})$ and $f$ twice differentiable and strongly convex.\\
