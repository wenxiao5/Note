...
\begin{enumerate}
    ...
    \begin{enumerate}[$\bullet$]
        ...
        \end{enumerate}
    \end{enumerate}
    \textbf{Computational cost:} After $k$ iterations, $\mathbb{E}[f(W_k)]\leq \min_W f(W)+\rho$, for $k=O(\frac{1}{\rho})$ and $f$ twice differentiable and strongly convex. The cost per iteration is $O(d)$ (independent of $n$), so the total cost is $O(\frac{d}{\varepsilon})$.
\end{enumerate}
