...
\section{Bootstrap}
\underline{Problem:} analyze the performance of an estimator $\hat{\theta}_n(\vec{Y})$, $\vec{Y}=(Y_1,Y_2,...,Y_n)$ taken i.i.d. from distribution $P$. e.g. $P_{\theta}=N(0,1),\hat{\theta}=\frac{1}{n}\sum_{i=1}^nY_i$

Assume $\theta$ is a scalar parameter. Performance: (1) Bias $\mathbb{E}[\hat{\theta}(\vec{Y})]=\theta$; (2) Variance $\mathbb{E}[\hat{\theta}^2(\vec{Y})]-\mathbb{E}^2[\hat{\theta}(\vec{Y})]$
