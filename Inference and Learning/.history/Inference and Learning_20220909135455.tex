...
\begin{center}
    ...
    \begin{enumerate}[$\bullet$]
        ...
        \begin{enumerate}[$\cdot$]
            \item If the prediction is $0$ and truth is $1$ ($\hat{y}=0|y=1$), add feature vector to weight vector.
            \item If the prediction is $1$ and truth is $0$ ($\hat{y}=1|y=0$), subtract feature vector from the weight vector.
        \end{enumerate}
    \end{enumerate}
    }}
\end{center}
...
\begin{enumerate}[$\bullet$]
    ...
    \item Under each training epoch:
    \begin{enumerate}[$\cdot$]
        \item Compute for each sample $\left\langle x^{(i)},y^{(i)}\right\rangle\in D$
    \end{enumerate}
\end{enumerate}
