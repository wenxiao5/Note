...
\begin{enumerate}[$\bullet$]
    \item \textbf{Original: }$$W=W+\eta(y_j-z)x_j$$
    \item \textbf{Unit-norm: }$$W=W+\eta(y_j-z)\frac{x_j}{\|x_j\|}$$
    where $\|x\|=\sqrt{x_1^2+x_2^2+\cdots+x_m^2}$
\end{enumerate}

\textbf{The Perceptron and ADALINE use variants of the delta rule!}
\begin{enumerate}[(1)]
    \item Perceptron: Output used in delta rule is $\hat{y}=g(x^TW)$; $w=w+\eta(y_j-\hat{y})x_j$
    \item ADALINE: Output used to estimate weights is $z=x^TW$.
\end{enumerate}
