...
\begin{definition}
    Deep feedforward networks, feedforward neural networks, multilayer perceptrons (MLPs), or dense neural networks \text{form the foundations of deep learning models. Learning occurs in only one direction:} \textbf{forward}. \text{There are no cycles or loops present. Information must flow from the input layer, through one or more hidden layers, before reaching the output.}
\end{definition}
