...
Generate $k$ resamples of $\vec{E}=(E_1,E_2,...,E_n)$ $\Rightarrow$ obtain $\vec{E^*}^{(1)},\vec{E^*}^{(2)},...,\vec{E^*}^{(k)}$ by resampling $\Rightarrow$ Compute pseudo-data $\vec{Y_i^*}^{(j)}=\hat{a}_n+\hat{b}_n\frac{i}{n}+\vec{E_i^*}^{(j)}$
