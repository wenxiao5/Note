...
\end{definition}
\textbf{Example of activation function:} $\text{identity}(x)=I(x)=x$; $\text{binary}(x)=\text{step}(x)=\left\{\begin{matrix}
    1,&x\geq 0\\
    0,&x<0
\end{matrix}\right.$
Since we want the prediction to be either $0$ or $1$, we usually use binary function as the activation function.
...
In perceptron, we generate the estimation $\hat{y}$ (after binary function) to help update weight $\{w_i\}_{i=0}^m$. However, in ADALINE, we minimize MSE to update weight $z=\{w_i\}_{i=0}^m$ before output estimation $\hat{y}$ (before binary function).

Before entering threshold (binary function), we want to minimize a mean-
squared error (MSE) loss
function to estimate weights.

e.g. suppose $g(x)=x$, let $\hat{y}=x^TW$ be the input of threshold, for each $y$,$$w_i=\argmin_{w_i}L(\hat{y},y)=(y-\hat{y})^2$$
\begin{equation}
    \begin{aligned}
        \frac{\partial L(\hat{y},y)}{\partial w_i}=-2(y-\hat{y})\frac{\partial \hat{y}}{\partial w_i}=-2(y-\hat{y})x_i
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{enumerate}[$\bullet$]
    \item \textbf{Original: }$$w=w-2\eta(y_j-\hat{y})x_j$$
    \item \textbf{Unit-norm: }$$w=w-2\eta(y_j-\hat{y})\frac{x_j}{\|x_j\|}$$
