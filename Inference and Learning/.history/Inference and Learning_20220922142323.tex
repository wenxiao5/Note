...
Assume $\theta$ is a scalar parameter. Performance: (1) Bias $\mathbb{E}[\hat{\theta}_n(\vec{Y})]=\theta$; (2) Variance $\mathbb{E}[\hat{\theta}_n^2(\vec{Y})]-\mathbb{E}^2[\hat{\theta}_n(\vec{Y})]$; (3) CDF $G_{n}(t)=P(\hat{\theta}_n(\vec{Y})<t),\forall t$

\subsubsection*{Approach \#1} Monte-Carlo Simulations:

Generate $k$ vectors $\vec{Y}^{(i)},i=1,2,...,k$ (total $kn$ random vaiables)
