...
Compute the expetation $$\mathbb{E}_p[h(x)]=\int_X \underbrace{p(x)h(x)}_{f(x)}dx$$

We want evaluate (1) $\int_X f$; (2) the probability of a set $a\subset X :P(a)=\int_a p(x)dx$, $h(x)=\mathbf{1}_{x\in a}$
...
$$\Rightarrow \hat{I}_n-I\leq \int_a^b\frac{b-a}{n}cdx=\frac{(b-a)^2}{n}c$$
