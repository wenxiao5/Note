...
\subsubsection{Proposition 1.4.2: properties of integer division}
...
\subsubsection{Prime, Greatest common divisor $gcd(a,b)$}
...
\subsubsection{Proposition 1.4.7: Euclidean Algorithm}
...
\subsubsection{Proposition 1.4.8:$gcd(a,b)$ exists and is the smallest positive integer in the set $M=\{ma+nb|m,n\in\mathbb{Z}\}$}
\begin{proposition}[Proposition 1.4.8]
    $\forall a,b\in\mathbb{Z}$, not both 0, $gcd(a,b)$ exists and is the smallest positive integer in the set $M=\{ma+nb|m,n\in\mathbb{Z}\}$.
\end{proposition}
i.e. $\exists m_0,n_0\in\mathbb{Z}$ s.t. $gcd(a,b)=m_0a+n_0b$.
\begin{proof}
\quad\\
...
$d-qc=r=(m-qm_0)a+(n-qn_0)b\in M$. So $r=0$. $\Rightarrow d=qc$. So $c|d$.\\
$a=1a+0b\in M\Rightarrow c|a$, $b=0a+1b\in M\Rightarrow c|b$.\\
If $t|a,t|b$ then $t|m_0 a+n_0 b\Rightarrow t|c$.\\
$\Rightarrow c=gcd(a,b)$.
\end{proof}


\subsubsection{Proposition 1.4.10: $b,c$ are \textit{relatively prime}, $b|ac$ $\Rightarrow$ $b|a$}
...
\subsubsection{Theorem 1.4.3: Any integer
$a > 1$ has a unique prime factorization}
\begin{lemma}[Lemma 1.4.4][Existence of Theorem 1.4.3]
    Any integer $a > 1$ has a prime factorization
\end{lemma}
Proved by induction.
\begin{theorem}[Theorem 1.4.3]
\end{theorem}
Any integer
$a > 1$ has a unique prime factorization:
...
\begin{proof}
\quad\\
Existence(Lemma 1.4.4)\\
Uniqueness: $a=p_1^{n_1}\cdot p_2^{n_2}\cdot...p_k^{n_k}=q_1^{r_1}\cdot q_2^{r_2}\cdot...q_j^{r_j}$.\\
$p_1>p_2>...>p_k, q_1>q_2>...>q_j, n_i,r_i\geq 1$\\
$p_1|a\Rightarrow \exists\ q_i\ s.t.\ p_1|q_i$. Similarly, $\exists\ q_i\ s.t.\ q_1|p_{i'}$\\
$q_1\leq p_i'\leq p_1\leq q_i\Rightarrow q_1= p_i'= p_1= q_i$\\
We can also know $n_1=r_1$, otherwise we would have two prime factorizations of the quotient where the largest primes are different by dividing $p_1^{\min\{n_1,r_1\}}$.\\
