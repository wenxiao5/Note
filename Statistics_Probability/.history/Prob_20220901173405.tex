...
\begin{equation}
    \begin{aligned}
        M_{X+Y}(t)=\mathbb{E}e^{t(X+Y)}=\mathbb{E}e^{tX}\mathbb{E}e^{tY}=e^{t(\mu_1+\mu_2)+\frac{1}{2}(\sigma_1^2+\sigma^2)t^2}=M_{Z}(t)
    \end{aligned}
    \nonumber
\end{equation}
...
\subsubsection{Multivariate (Joint) Gaussian (Normal) Distribution}
A $k$-dimensional random vector $(X_1,X_2,...,X_k)^T=\mathbf{X}\sim N(\mu,\Sigma)$
\begin{enumerate}[(1)]
    \item $\mu$ is a $k$-dimensional mean vector: $$\mu=\mathbb{E}[\mathbf{X}]=(\mathbb{E}[X_1],\mathbb{E}[X_2],...,\mathbb{E}[X_k])^T$$
    \item $k\times k$ covariance matrix
    $${\displaystyle \Sigma _{i,j}=\operatorname {E} [(X_{i}-\mu _{i})(X_{j}-\mu _{j})]=\operatorname {Cov} [X_{i},X_{j}]}{\displaystyle \Sigma _{i,j}=\operatorname {E} [(X_{i}-\mu _{i})(X_{j}-\mu _{j})]=\operatorname {Cov} [X_{i},X_{j}]}$$
\end{enumerate}
...
Given a time homogeneous MC with initial distribution $X\sim\alpha\in [0,1]^{|S|}$ and transition matrix $P$.
...
\subsubsection*{Chapman Kolmogorov Equations (C-K Equations)}
...
\begin{definition}
Markov Chain is said to have \textbf{limitating distribution $\lambda$} if $\forall i,j\in S$ we have $\lim_{n\in \infty}P^n_{ij}=\lambda_j$ or $\forall \alpha$, $\lim_{n\in \infty}\alpha P^n=\lambda$
\end{definition}
