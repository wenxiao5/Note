...
\subsubsection{Long Run Behavior of Finite Markov Chains}
...
\begin{definition}
    A MC is said to have a limiting distribution $\lambda$ if we have $$\lim_{n \rightarrow \infty}P_{ij}^n=\lambda_j,\ \forall i,j\in S$$
    An \textbf{equivalent definition} is that for all initial distributions $X_0\sim \alpha$ and all $j\in S$ we have
