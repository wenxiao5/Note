...
\subsubsection{Fundamental Theorem for \underline{Irreducible, Finite MC}: $\pi_j=\frac{1}{\mathbb{E}(T_j|X_0 = j)}$}
...
\subsubsection{Expected Number of Visits to a Transient State}
...
\end{lemma}
\begin{proof}
    We can write $$E(Y_i|X_0 = j)=\sum_{n=0}^\infty P(X_n=i|X_0=j)=\sum_{n=0}^\infty P^n_{ji}=\sum_{n=0}^\infty Q^n_{ji}=M_{ji}$$
    The last equality holds because $I+Q+Q^2+\cdots=\frac{I(I-Q^\infty)}{1-Q}=(I-Q)^{-1}$
\end{proof}

\subsubsection{Expected Time till Absorption to a Recurrent Class}
...
\end{bmatrix}$, then $$M=(I-Q)^{-1}=\begin{bmatrix}
    ...
    1/2& 1 & 3/2
\end{bmatrix}$$
Therefore $\mathbb{E}(Y_3|X_0=1)=\frac{1}{2}$, $\mathbb{E}(T_{abs}|X_0=1)=M_{11}+M_{12}+M_{13}=\frac{3}{2}+1+\frac{1}{2}$
