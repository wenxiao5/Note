...
\begin{enumerate}
    ...
    \item All binary sequence consisted of $0$ or $1$ of length $d$ and \textbf{no consecutive 1}, $B=\{0,1\}^d$, $|B|=2^d$. $\pi$ is a uniform distribution on all $B$, but it is hard to get the $|B|$ is this situation.
    
    Ejection Sampling: sample a random sequence from $S$, eject the sequence is not in $B$.

    We can use a MC with $P_{00}=P_{0,1}\frac{1}{2},P_{1,1}=0, P_{1,0}=1$.
\end{enumerate}
