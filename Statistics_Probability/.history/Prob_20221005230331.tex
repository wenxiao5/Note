...
We have $G_n(s)=\psi_{Z_n}(s)=\mathbb{E}(s^{Z_n})= \mathbb{E}\left(\mathbb{E}(s^{\sum_{k=1}^{Z_{n-1}}X_k})|Z_{n-1}\right)=\mathbb{E}\left(\prod_{k=1}^{Z_n-1}\mathbb{E}(s^X_k)|Z_{n-1}\right)$
