...
\subsection{Example}
\begin{enumerate}
    \item All binary sequence consisted of $0$ or $1$ of length $d$, $X=\{0,1\}^d$, $|X|=2^d$. $\pi$ is a uniform distribution on all $X$, obviously, each sequence has probability $\frac{1}{2^d}$. We can use a MC of tossing a coin to simulate it.
    \item All binary sequence consisted of $0$ or $1$ of length $d$ and \textbf{no consecutive 1}, $X=\{0,1\}^d$, $|X|=2^d$. $\pi$ is a uniform distribution on all $X$, but it is hard to get the $|X|$ is this situation. We can use a MC with $P_{00}=P_{0,1}\frac{1}{2},P_{1,1}=0, P_{1,0}=1$.
\end{enumerate}
