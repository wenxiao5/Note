...
\subsubsection{Expected Number of Visits to a Transient State: $E(Y_i|X_0 = j) = (I-Q)^{-1}_{ji}$}
...
This problem can be transformed into computing the \textbf{expected time till absorption to $i$}. (we can let $i$ be an absorbing state)

Reorder the transition matrix $P$ with $i$ being the first state: $$P=\begin{bmatrix}
    P_{ii}& R\\
    S & Q
\end{bmatrix}$$
