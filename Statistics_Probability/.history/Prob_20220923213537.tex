...
\begin{equation}
    \begin{aligned}
        f_i&=\sum_{n=1}^\infty P(T_i=n)=\sum_{n=1}^\infty\left(\frac{1}{n}-\frac{1}{n+1}\right)\\
        \mu_i&=\sum_{n=1}^\infty nP(T_i=n)=\sum_{n=1}^\infty\left(1-\frac{1}{n+1}\right)
    \end{aligned}
    \nonumber
\end{equation}
...
Obviously, when a chain is irreducible, $\lim_{n \rightarrow \infty}P^{n}_{y,x}=0$, there will be no limiting distribution.

\begin{lemma}
    For an irreducible MC, $\lim_{n \rightarrow \infty}P^{n}_{y,x}=0$ for each $x,y\in S$ iff the chain is transient or null recurrent.
\end{lemma}
...
\begin{theorem}[Fundamental Theorem for General Discrete Markov Chains]
    ...
    The stationary distribution $\pi$ is also \textbf{inversely} related to the \textbf{expected first return times}. $$\pi(j)=\frac{1}{\mathbb{E}(T_j|X_0=j)}$$
