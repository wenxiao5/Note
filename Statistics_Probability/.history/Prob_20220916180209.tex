...
When an MC is irreducible but \textbf{periodic}, we can prove there is no \textbf{limiting distribution}.
\begin{lemma}
    Suppose an MC is periodic (period $d>1$), there is no limiting distribution.
\end{lemma}
\begin{proof}
    Assume there exists a limiting distribution $\pi$ such that $$\lim_{n \rightarrow \infty}P_{ij}^n=\lambda_j,\ \forall i,j\in S$$
    And by periodic we know $P_{i,i}=0,\forall i\in S$
    \begin{equation}
        \begin{aligned}
            \lim_{n \rightarrow \infty}P_{ij}^{n+d-1}&=\sum_{k\in S}\lambda_kP^{d-1}_{kj}=\sum_{k\neq j}\lambda_kP^{d-1}_{kj}=\lambda_j\\
            \lim_{n \rightarrow \infty}P_{ij}^{n+d}&=\sum_{t\in S}\lim_{n \rightarrow \infty}P_{it}^{n+d-1}P_{tj}=\sum_{t\in S}(\sum_{k\neq t}\lambda_kP^{d-1}_{kt})P_{tj}=\sum_{k\neq t}\lambda_k\sum_{t\in S}P^{d-1}_{kt}P_{tj}=\sum_{k\neq t}\lambda_kP^{d}_{kj}
        \end{aligned}
        \nonumber
    \end{equation}
    We also know
    \begin{equation}
        \begin{aligned}
            \lim_{n \rightarrow \infty}P_{ij}^{n+d}&=\sum_{k\in S}\lambda_kP^{d}_{kj}
        \end{aligned}
        \nonumber
    \end{equation}
\end{proof}
