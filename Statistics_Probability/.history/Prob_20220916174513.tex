...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \lim_{n \rightarrow \infty}P_{ij}^{n+1}=\sum_{k\in S}\lambda_kP_{kj}=\sum_{k\neq j}\lambda_kP_{kj}=\lambda_j\\
            \Rightarrow \sum_{j\in S}\sum_{k\neq j}\lambda_kP_{kj}=\sum_{j\in S}\lambda_j \Rightarrow \sum_{k\neq j}\lambda_k=\sum_{j\in S}\lambda_j
        \end{aligned}
        \nonumber
    \end{equation}
\end{proof}
