...
\begin{enumerate}[$\bullet$]
    ...
    \item \underline{Recurrent}: State $i$ is recurrent if $f_i=P($ever re-enter state $i$ if started in state $i)=1$. (the expected number of times it visits state $i$ is infinite). (A MC is irreducible if all states are recurrent)
    \item 
\end{enumerate}
