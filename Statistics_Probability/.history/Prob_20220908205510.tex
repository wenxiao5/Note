...
\begin{enumerate}[$\bullet$]
    ...
    \begin{lemma}
        If $i$ is recurrent, $i\leftrightarrow j \Rightarrow$ $j$ is recurrent. (The states of a communication class are either all recurrent or all transient.)
    \end{lemma}
    \begin{lemma}
...
A set of states $C$ is said to be \underline{closed} if no state outside of $C$ is accessible from any state in $C$. If $C$ is closed, then $P_{ij} = 0$ for all $i\in C$ and $j\notin C$.
