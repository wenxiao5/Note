...
\begin{enumerate}[$\bullet$]
    ...
    \item \underline{Transient}: State $i$ is transient if $f_i=P($ever re-enter state $i$ if started in state $i)<1$. (the expected number of times it visits state $i$ is \textit{finite}, $P($visits state $i$ exactly $n$ times$)=f_i^{n-1}(1-f_i)$).
\end{enumerate}
