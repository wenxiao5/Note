...
\subsection{Basics}
\underline{\textbf{Neuron}:} Neuron is a non-linear function, which takes $\mathfrak{J}\in \mathbb{R}$ as input and produce $\sigma(\mathfrak{J})\in \mathbb{R}$ as output.
\begin{center}\begin{figure}[htbp]
    \centering
...
\begin{center}\begin{figure}[htbp]
    ...
    \label{}
\end{figure}\end{center}
Given a vector input $x$, we need to find the best estimator $\hat{y}$ which minimizes $y$. In the figure that has only one layer and one pathway, we find the parameter $(\omega,b)$ to form an input $\omega^Tx+b$ to neuron $\sigma$.  Then, the final output (estimator) of the network is $\hat{y}=\sigma(\omega^Tx+b)$.
