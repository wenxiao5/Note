...
\begin{center}
    \fcolorbox{black}{blue!5}{\parbox{.9\linewidth}{\begin{lemma}[Sensitivity vector lemma]
        Let $P$ be a convex program with a point $\vec{x}^*\in S$ such that $\vec{g}(\vec{x}^*)< \vec{0}$ (Slater's condition, which make sure $\vec{0}$ is an interior point of $MP$'s domain). Then there exists some $\vec{\lambda}\in \mathbb{R}^m$ with $\vec{\lambda}\geq \vec{0}$ such that $$MP(\vec{z})\geq MP(\vec{0})-\vec{\lambda}\cdot \vec{z},\quad \forall \vec{z}$$
