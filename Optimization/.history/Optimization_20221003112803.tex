...
For $\vec{x}\in \mathbb{R}^n, \vec{y}\in \mathbb{R}^m$The least-square error problem can be written as find $A\in \mathbb{R}^{m\times n}$ to minimize $\|A\vec{x}-\vec{y}\|$.
...
\begin{lemma}
    If $V=\{A\vec{x}:\vec{x}\in \mathbb{R}^n\}$, then the point $(A\vec{x})^*\in V$ is the closest point of $V$ to $\vec{y}\in \mathbb{R}^m$ if and only if $$((A\vec{x})^*-\vec{y})\bot \vec{a}$$
    for all $\vec{a}\in V$
\end{lemma}
