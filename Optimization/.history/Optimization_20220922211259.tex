...
\end{enumerate}
In general, we want to eliminate all $t_1,...,t_n$ is the RHS of the inequality and only left the $\left(\frac{C_1}{\delta_1}\right)^{\delta_1}\cdots\left(\frac{C_n}{\delta_n}\right)^{\delta_n}$

\textbf{Dual Geometric Problem}
\begin{equation}
    \begin{aligned}
        \max_{\vec{\delta}\in \mathbb{R}^n_{>0}}\quad V(\vec{\delta})=\left(\frac{C_1}{\delta_1}\right)^{\delta_1}\cdots\left(\frac{C_n}{\delta_n}\right)^{\delta_n}
    \end{aligned}
    \nonumber
\end{equation}
