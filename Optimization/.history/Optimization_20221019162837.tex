...
\begin{proof}
    ...
    The points $\vec{u}^{(1)},\vec{u}^{(2)},...$ are a sequence of points in the closed and bounded set $\{x \in \mathbb{R}^n : \|x\|= 1\}$. This means we can pick out a subsequence of them converging to some u ∈ Rn with u= 1. Taking the limit of the inequality u(k) · x < u(k) ·y(k) for this subsequence, we get u·x ≤ u·z for all x ∈ C, which was what we wanted.
\end{proof}
