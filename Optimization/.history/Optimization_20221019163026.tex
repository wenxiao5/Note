...
\begin{proof}
    ...
    Taking the limit of the inequality $\vec{u}^{(k)}\cdot \vec{x}<\vec{u}^{(k)}\cdot \vec{y}^{(k)}$ for this subsequence, we get $$\vec{u}\cdot \vec{x}<\vec{u}\cdot \vec{z},\ \forall \vec{x}\in C$$
\end{proof}
