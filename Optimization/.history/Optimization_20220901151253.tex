...
$$\hat{\delta}_B=\argmin_\delta \frac{1}{n}\sum_{i=1}^nl(\delta(X_i),Y_i)$$
The corresponding risk is $R(\hat{\delta})=\mathbb{E}_P\left[l(\hat{\delta}(X),Y)\right]$. $\Delta R(\hat{\delta})=R(\hat{\delta})- R(\delta)>0$ always holds.

\textbf{Consistency:} if $\Delta R(\hat{\delta}) \rightarrow 0$ as $n \rightarrow \infty$.
