...
\begin{enumerate}[$\bullet$]
    ...
    \begin{enumerate}[(1)]
        ...
        \begin{equation}
            ...
            \nonumber
        \end{equation}
        which contradicts to $x_1^2+x_2^2=5$.
    ...
    \end{enumerate}
    Hence, the only candidate sstisfy the first order condition is $x^*=(1,2)$, $\mu=(1,0)$
...
\subsubsection{Weak Duality Theorem: $\max_{(\lambda,\mu)\in G}D(\lambda,\mu)\leq \min_{x\in F}f(x)$}
...
\subsubsection{Strong Duality Theorem: under some conditions, $\max_{(\lambda,\mu)\in G}D(\lambda,\mu)= \min_{x\in F}f(x)$}
...
\end{equation}
\begin{proposition}
    Suppose $f$ is \underline{convex}, $h_i$ are \underline{affine}, $g_j$ are \underline{convex}, and \underline{$\&=\mathbb{R}^n$}. If $x^*$ is an optimal solution for primal problem, $x^*$ is regular, and $(\lambda^*,\mu^*)$ are corresponding Largrange multipliers, then strong duality holds and $(\lambda^*,\mu^*)$ maximize $D(\lambda,\mu)$.
\end{proposition}
\begin{proof}
    ...
    $L(x^*,\lambda^*,\mu^*)$ is convex in $x$. Thus, $L(x^*,\lambda^*,\mu^*)=\min_{x\in \mathbb{R}^n}L(x,\lambda^*,\mu^*)$
\end{proof}
