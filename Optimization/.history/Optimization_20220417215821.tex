...
\end{example}
\textbf{Note:} that if $g_j(x)$ is \underline{convex} for all $j$, then both of their barrier functions are \underline{convex}.
...
Therefore, if we have a initial point in the interior of $F$, we can choose step size of any unconstrained GD method to stay in interior of $F$ for all iterations and solve the ICP.
