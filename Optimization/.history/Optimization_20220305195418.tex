...
For large $M$, recall $y_i^{(M)}=\sigma(x_i^{(M)})$, $x_i^{(M)}=\sum_jw_{ij}^{(M)}y_j^{(M-1)}+b_i^{(M)}$
\begin{equation}
    \begin{aligned}
...
\begin{enumerate}[$\bullet$]
    ...
    \item $\frac{\partial y_i^{(M)}}{\partial x_{i}^{(M)}}=\frac{\partial \sigma(x_{i}^{(M)})}{\partial x_{i}^{(M)}}=\sigma'(x_{i}^{(M)})$
\end{enumerate}
