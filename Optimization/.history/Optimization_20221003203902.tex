...
\end{lemma}
As we know in algebra, vectors in null space ($N(A)=\{\vec{x}:A \vec{x}=\vec{0}\}$) are orthogonal to all of the vectors in the row space (spanned by rows of $A$). $\vec{x}^TA^T=\vec{0}^T$
