...
Then, the question is how to make $\lambda$ close to $\lambda^*$ without knowing $\lambda^*$? \textbf{Duality!}

$$(P):\ \min f(x)\text{ s.t. }h(x)=0$$

\begin{equation}
    \begin{aligned}
        Lagrangian:& L(x,\lambda)=f(x)+\lambda^Th(x)\\
        Dual:& D(\lambda)=\min_x L(x,\lambda)
    \end{aligned}
    \nonumber
\end{equation}
Lagrangian: $L(x,\lambda)=f(x)+\lambda^Th(x)$
