...
When solving the closest point problem for a vector space $S=\{\vec{x}:A \vec{x}=\vec{0}\}$, we use \textbf{perpendicularity condition} (orthogonality condition) -i.e., that $\vec{x}^*\cdot \vec{y}=0,\forall \vec{y}\in S$
