...
\begin{enumerate}[(i)]
    ...
    \item If $f(x_k+\alpha_k d_k)\leq f(x_k)+\sigma \alpha_k \nabla f(x_k)^T d_k$, stop.
    ...
    Termination at \underline{smallest integer $m$} s.t. $$f(x_k+\beta^m \tilde{\alpha} d_k)\leq f(x_k)+\sigma\beta^m \tilde{\alpha}\nabla f(x)^T d_k$$
\end{enumerate}
