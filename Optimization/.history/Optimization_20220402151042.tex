...
\subsubsection{Prop: $[z]^\&$ is projection on \underline{closed conex} subset of $\mathbb{R}^n$$\Leftrightarrow$ $(z-[z]^\&)^T(y-[z]^\&)\leq 0, \forall y\in\&$}
\begin{proposition}
...
\subsubsection{Prop: $L-$Lipschitz gradient and $0<\alpha<\frac{2}{L}$ $\Rightarrow$ limit point is a fixed point (in fixed step-size steepest descent method)}
\begin{proposition}
...
\begin{proof}
    \begin{equation}
        \begin{aligned}
            \text{$x^*$ is minimizer of convex $f$ over $\&$} &\Leftrightarrow \text{$\nabla f(x^*)^T(x-x^*)\geq 0,\forall x\in \&$}\\
            &\Leftrightarrow \text{$-\alpha\nabla f(x^*)^T(x-x^*)\leq 0,\forall x\in \&$}\\
            &\Leftrightarrow \text{$(x^*-\alpha\nabla f(x^*)-x^*)^T(x-x^*)\leq 0,\forall x\in \&$}\\
        \end{aligned}
        \nonumber
    \end{equation}
\end{proof}
