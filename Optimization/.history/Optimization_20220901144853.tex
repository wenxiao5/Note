...
\subsubsection{$\mathbb{R}^n$ example}
\begin{example}
...
In this rule the whole optimization problem also goes to
$$\delta_{MMSE}(x)=\int_y \pi(y|x) (y-\mathbb{E}\left[Y|X=x\right])^2 dy=Var\left[Y|X\right]$$
$$R(\delta_{MMSE})=\int_x P(x)\int_y \pi(y|x) (y-\mathbb{E}\left[Y|X=x\right])^2 dy dx=\mathbb{E}_X Var\left[Y|X\right]$$
