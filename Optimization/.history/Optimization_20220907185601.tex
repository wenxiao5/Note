...
\end{corollary}
Except proving $f(x)=|x|^p,p\geq 1$ is convex we can also prove $f(x)=|x|^p,p\geq 1$ is strongly convex.
\begin{corollary}
    $f(x)=|x|^p,p\geq 1$ is strongly convex.
\end{corollary}
\begin{proof}
$F(x)=f(x)-mx^2=|x|^p-\frac{m}{2}x^2$. $g_1(x)=|x|,x\in \mathbb{R}^n$ is convex; $g_2(x)=x^p-mx^2,x\in \mathbb{R}^+$ is also convex since $g_2^{''}(x)=p(p-1)x^{p-2}-m,$
\end{proof}
