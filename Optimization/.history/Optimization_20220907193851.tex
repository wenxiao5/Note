...
\begin{center}
    \fcolorbox{black}{gray!10}{\parbox{.9\linewidth}{\underline{\textbf{Convex function} (0-th order)}: $f$ is convex in a convex set $C$ iff $f(\alpha x+(1-\alpha) y) \leq \alpha f(x)+(1-\alpha) f(y), \forall x, y \in C, \forall \alpha \in[0,1] .$
...
\textbf{Alternative 1 ($1^{st}$ order)}: If $f$ is differentiable, then $f$ is convex iff $f(z) \geq f(x)+(z-x)^{T} \nabla f(x), \ \forall x, z \in C .$ The inequality is strict for strict convexity.
\begin{proof}
\quad
\begin{enumerate}[(i)]
...
\textbf{Alternative 2 ($2^{nd}$ order)}: If $f$ is twice differentiable, then $f$ is convex iff
$$
\nabla^{2} f(x) \succeq 0,\ \forall x \in C .
$$
...
\subsubsection{Theorem: If $f$ is convex and $g$ is convex and increasing, $(g\cdot f)(x)$ is convex}
\begin{theorem}
    If $f$ is convex and $g$ is convex and increasing, $(g\cdot f)(x)=g(f(x))$ is convex.
\end{theorem}
\begin{proof}
...
\end{corollary}
Except proving $f(x)=|x|^p,p\geq 1$ is convex we can also prove $f(x)=|x|^p,p> 1$ is strongly convex in $[-1,1]$.
\begin{corollary}
    $f(x)=|x|^p,p> 1$ is strongly convex in $[-1,1]$.
\end{corollary}
\begin{proof}
$F(x)=f(x)-mx^2=|x|^p-\frac{m}{2}x^2$. $g_1(x)=|x|,x\in [-1,1]$ is convex; $g_2(x)=x^p-\frac{m}{2}x^2,x\in [0,1]$ is also convex: $g_2(y)+(x-y)g_2^{'}(y)=y^p-\frac{m}{2}y^2+(x-y)(py^{p-1}-my)=(1-p)y^p+pxy^{p-1}+\frac{m}{2}y^2-mxy$. We let $G_x(y)=g_2(y)+(x-y)g_2^{'}(y), \frac{\partial G_x(y)}{\partial y}=xg^{''}_2(y)-yg^{''}_2(y)=(x-y)g^{''}_2(y)$



$-(p-1)py^{p-1}+(p-1)py^{p-2}x+my-mx=\left(p(p-1)y^{p-2}-m\right)(x-y) \Rightarrow$ (Suppose $m$ is small enough) $G_x(y)$ is decreasing in $(0,\sqrt[p-2]{\frac{m}{p(p-1)}}]$ and $[x,1]$ and increasing in $[\sqrt[p-2]{\frac{m}{p(p-1)}},x]$
\end{proof}
...
\section{Optimization in $\mathbb{R}^n$: Restriction to a Line}
\subsection{Restriction to a Line}
\subsubsection{Definition: $\phi_{\vec{u}}(t)=f(\vec{x}+t\vec{u})$}
\begin{definition}
...
\subsubsection{Derivatives: $\phi'_{\vec{u}}(t)=\nabla f(\vec{x}+t\vec{u})\cdot \vec{u}$, $\phi^{''}_{\vec{u}} (t)=\vec{u}^T {Hf}(\vec{x}+t\vec{u})\vec{u}$}
...
\subsubsection{Lemma: $x^*$ is a global minimizer of $f$ $\Leftrightarrow$ $t=0$ is the global minimizer of $\phi_{\vec{u}}(t)=f(\vec{x}+t\vec{u})$}
\begin{lemma}
...
\subsection{Minimizer Conditions}
\subsubsection{Global-min Necessary Condition: $\nabla f$ is continuous, $x^*$ is a global minimizer of $f$ $\Rightarrow \nabla f(x^*)=0$}
\begin{theorem}
...
\subsubsection{Global-min Sufficient Condition: $Hf$ is continuous, $\nabla f(\vec{x}^*)=0$, $\vec{u}^T Hf(\vec{x}) \vec{u}\geq 0,\forall \vec{x},\vec{u}\in \mathbb{R}^n$ $\Rightarrow$ $\vec{x}^*$ is a global minimizer of $f$}
\begin{theorem}
...
\subsubsection{Local-min Necessary Condition: $\nabla f$ is continuous, $x^*$ is a local minimizer of $f$ $\Rightarrow \nabla f(x^*)=0$}
...
\subsubsection{Local-min Sufficient Condition: $Hf$ is continuous, $\nabla f(\vec{x}^*)=0$, $\vec{u}^T Hf(\vec{x}) \vec{u}\geq 0,\forall \vec{u}\in \mathbb{R}^n$ and $\|\vec{x}-\vec{x}^*\|<r$ $\Rightarrow$ $\vec{x}^*$ is a local minimizer of $f$}
\begin{theorem}
    Given a function $f:\mathbb{R}^n \rightarrow \mathbb{R}$, if $Hf$ is continuous and $\vec{x}^*$ is a critical point of
...
\subsubsection{$Hf(\vec{x}^*)\succ 0$/$\prec 0$ $\Rightarrow$ $\vec{x}^*$ is local-min/local-max}
