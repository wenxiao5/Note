...
\begin{proposition}
Assume $\inf_x f(x)>-\infty$. Then every limit point of $\{x_k\}$ for steepest descent with Armijo's rule is a stationary point of $f$.
\end{proposition}
\begin{proof}
...
\begin{enumerate}[$\bullet$]
    ...
    $$ln\alpha_k=ln (\tilde{\alpha}\beta^{m_k})=ln\tilde{\alpha}+m_kln\beta \Rightarrow	m_k=\frac{ln\alpha_k-ln\tilde{\alpha}}{ln\beta}$$
\end{enumerate}
\end{proof}
