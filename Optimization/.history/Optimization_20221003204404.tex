...
\end{lemma}
Obviously, \textbf{all} vectors in null space ($N(A)=\{\vec{x}:A \vec{x}=\vec{0}\}$) are orthogonal to a vector \textbf{if and only if} it is a linear combination of $A$'s rows.
