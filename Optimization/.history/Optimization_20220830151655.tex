...
\underline{Loss function} with action (estimation) $a$ is $l(a,\theta)$. e.g. (binary HT) Hamming/zero-one loss $l(a=\hat{\theta},\theta)=\mathbf{1}_{a\neq \theta}$ e.g. (estimation) $l(a=\hat{\theta},\theta)=(a-\theta)^2$ squared error loss $l(a,\theta)=|a-\theta|$ absolute error loss.

Risk of decision rule $\delta$: $$R(\delta)=\mathbb{E}\left(l(\delta(X),\theta)\right)$$ where $(X,\theta)$ are random with prob $\pi(\theta),P_\theta$

\begin{definition}
The Bayes rule is $\delta_B=\argmin_\delta n(\delta)$
\end{definition}
