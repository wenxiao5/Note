...
\subsubsection{Lagrange Mutiplier Theorem: First-order necessary condition: $\exists\lambda, \nabla f(x^*)+\sum_{i=1}^m\lambda_i \nabla h_i(x^*)=0$}
...
\subsubsection{Lagrange Mutiplier Theorem: Second-order necessary condition}
\begin{theorem}
...
\begin{equation}
    ...
    \nonumber
\end{equation}
As $k \rightarrow \infty$, $x^{(k)} \rightarrow	x^*$, $kh(x^{(k)})\rightarrow-\left(\nabla h(x^{*})\right)^+\nabla f(x^{*})\triangleq \lambda	\lambda_i$ (is proved in first-order necessary condition part)
