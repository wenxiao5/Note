...
\begin{enumerate}[(1)]
    ...
    $G_x(0)=0$, $G_x(x)=g_2(x)=x^p-\frac{m}{2}x^2$. Since we know $g_2(x)\geq 0,\forall x\in [0,1]$ if $m\leq 2$, we can infer $G_x(y)\leq g_2(x)$. i.e., $g_2(y)+(x-y)g_2^{'}(y)\leq g_2(x),\forall x,y\in [0,1]$. Then we can infer $g_2(x)$ is convex and increasing for $m<p$.
\end{enumerate}
$g_2(y)+(x-y)g_2^{'}(y)=y^p-\frac{m}{2}y^2+(x-y)(py^{p-1}-my)=(1-p)y^p+pxy^{p-1}+\frac{m}{2}y^2-mxy$. We let $G_x(y)=g_2(y)+(x-y)g_2^{'}(y), \frac{\partial G_x(y)}{\partial y}=xg^{''}_2(y)-yg^{''}_2(y)=(x-y)g^{''}_2(y)$
