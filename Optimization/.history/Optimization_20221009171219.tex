...
\end{lemma}
Obviously, \textbf{all} vectors in null space ($N(A)=\{\vec{x}:A \vec{x}=\vec{0}\}$) are orthogonal to a vector \textbf{if and only if} it is a linear combination of $A$'s rows. $\vec{x}^*=A^T \vec{w}$, for some $\vec{w}\in \mathbb{R}^n$.
\begin{theorem}
...
\subsection{General Norm and its Application in Optimization}
\begin{definition}
...
\end{definition}
Solve the optimization probelm
\begin{equation}
    \begin{aligned}
        \min_{\vec{x}\in \mathbb{R}^n}\quad &\vec{x}^T H \vec{x}\\
        s.t. \quad&A \vec{x}=\vec{b}
    \end{aligned}
    \nonumber
\end{equation}
