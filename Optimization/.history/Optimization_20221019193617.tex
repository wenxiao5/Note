...
\begin{definition}
    If $C\subseteq \mathbb{R}^n$ is a convex set, $f: C \rightarrow  \mathbb{R}$ is a convex function, and $\vec{x}^*\in int(C)$. $\vec{d}$ is called a \textbf{sub-gradient} of $f$ at $\vec{x}^*$ if $$f(\vec{x})\geq f(\vec{x}^*)+\vec{d}\cdot (\vec{x}-\vec{x}^*),\ \forall \vec{x}\in C$$
\end{definition}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    (Specifically, we substitute $(\vec{x}^*,f(\vec{x}^*))$)
