...
\begin{definition}
    If $C\subseteq \mathbb{R}^n$ is a convex set, $f: C \rightarrow  \mathbb{R}$ is a convex function, and $\vec{d}$ is called a \textbf{sub-gradient} of $f$ at $\vec{x}^*$ if $$f(\vec{x})\geq f(\vec{x}^*)+\vec{d}\cdot (\vec{x}-\vec{x}^*),\ \forall \vec{x}\in C$$
\end{definition}
...
\begin{proof}
    We can use epigraphs. Recall that for a function $f: C \rightarrow \mathbb{R}$, $$epi(f)=\{(\vec{x},y)\in \mathbb{R}^{n+1}: y\geq -\sqrt{x}\}$$
    ...
    By the support theorem, there exists a vector $(\vec{a},b)\in \mathbb{R}^{n+1}$ with $\|(\vec{a},b)\|=1$ such that $$(\vec{a},b)\cdot \left(\vec{x}^*,f(\vec{x}^*)\right)\geq (\vec{a},b)\cdot \left(\vec{x},y\right),\ \forall \left(\vec{x},y\right)\in epi(f)$$
\end{proof}
