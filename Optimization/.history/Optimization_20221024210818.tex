...
Suppose that $P$ has an optimal solution $\vec{x}^*\in S$ and a sensitivity vector $\vec{\lambda}^*\geq \vec{0}$. Then for all $\vec{\lambda}\geq \vec{0}$, $$h(\vec{\lambda})\leq h(\vec{\lambda}^*)=f(\vec{\lambda}^*)$$
\end{theorem}
