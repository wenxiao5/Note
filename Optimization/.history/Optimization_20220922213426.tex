...
\end{enumerate}
In general, we want to eliminate all $t_1,...,t_n$ is the RHS of the inequality, then the RHS can be transformed into constant $\left(\frac{C_1}{\delta_1}\right)^{\delta_1}\left(\frac{C_2}{\delta_2}\right)^{\delta_2}\cdots\left(\frac{C_n}{\delta_n}\right)^{\delta_n}$
...
\begin{center}
    ...
    \begin{equation}
        \begin{aligned}
            &s.t.\quad& \delta_1\alpha_{1,1}+\delta_2\alpha_{1,2}+\cdots+\delta_n\alpha_{1,n}&=0\quad \text{(power of $t_1$)}\\
            &&\vdots\\
            &&\delta_1\alpha_{m,1}+\delta_2\alpha_{m,2}+\cdots+\delta_n\alpha_{m,n}&=0\quad \text{(power of $t_m$)}\\
...
Suppose $\vec{\delta}^*$ is the solution to the dual GP. $$\sum_{i=1}^n\delta^*_i\left(\frac{{Term}_i(t_1,\cdots,t_m)}{\delta^*_i}\right)\geq \left(\frac{{Term}_1(t_1,\cdots,t_m)}{\delta^*_1}\right)^{\delta^*_1}\cdots \left(\frac{{Term}_n(t_1,\cdots,t_m)}{\delta^*_n}\right)^{\delta^*_n}$$
The inequality holds only if
\begin{equation}
    \begin{aligned}
        \frac{{Term}_1(\vec{t})}{\delta_1}=\frac{{Term}_2(\vec{t})}{\delta_2}=\cdots=\frac{{Term}_m(\vec{t})}{\delta_m}=V(\vec{\delta}^*)
    \end{aligned}
    \nonumber
\end{equation}
