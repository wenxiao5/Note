...
\end{corollary}
\begin{proof}
$F(x)=f(x)-mx^2=|x|^p-\frac{m}{2}x^2$. $g_1(x)=|x|,x\in [-1,1]$ is convex; $g_2(x)=x^p-\frac{m}{2}x^2,x\in [0,1]$ is also convex: $g_2(y)+(x-y)g_2^{'}(y)=y^p-\frac{m}{2}y^2+(x-y)(py^{p-1}-my)=(1-p)y^p+pxy^{p-1}+\frac{m}{2}y^2-mxy$. We let $G_x(y)=g_2(y)+(x-y)g_2^{'}(y), \frac{\partial G_x(y)}{\partial y}=py^{p-1}-my-px^{p-1}+mx \Rightarrow$ $G_x(y)$ is increasing in $[0,x]$
\end{proof}
