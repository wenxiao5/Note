...
\begin{definition}
    A nonlinear program $P$ is a \textbf{convex program} if: 1. The set $S$ is convex; 2. All the function $f,g_1,...,g_m$.
\end{definition}
...
\begin{theorem}
    Let $P$ be a convex program. Then the domain of $MP(\vec{z})$ is feasible - is a convex set and $MP(\vec{z})$ is a convex function on that domain.
\end{theorem}
\begin{example}
...
\end{example}
\begin{enumerate}
    \item When $z<0$, there are \underline{no} feasible points, so $MP(z)=+\infty$ and were outside the domain of MP.
    \item When 
\end{enumerate}
