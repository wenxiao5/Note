...
\end{corollary}
\begin{proof}
$F(x)=f(x)-mx^2=|x|^p-\frac{m}{2}x^2$. $g_1(x)=|x|,x\in [-1,1]$ is convex; $g_2(x)=x^p-\frac{m}{2}x^2,x\in [0,1]$ is also convex: $g_2(y)+(x-y)g_2^{'}(y)=y^p-\frac{m}{2}y^2+(x-y)(py^{p-1}-my)=(1-p)y^p+pxy^{p-1}+\frac{m}{2}y^2-mxy$. We let $G_x(y)=g_2(y)+(x-y)g_2^{'}(y), \frac{\partial G_x(y)}{\partial y}=-(p-1)py^{p-1}+(p-1)py^{p-2}x+my-mx=\left(p(p-1)y^{p-2}-m\right)(x-y) \Rightarrow$ (Suppose $m$ is small enough) $G_x(y)$ is decreasing in $(0,\sqrt[p-2]{\frac{m}{p(p-1)}}]$ and $[x,1]$ and increasing in $[\sqrt[p-2]{\frac{m}{p(p-1)}},x]$
\end{proof}
