...
\end{definition}
The first derivative of $\phi_{\vec{u}}(t)=f(\vec{x}+t\vec{u})$, $\phi'_{\vec{u}}(t)=\nabla f(\vec{x}+t\vec{u})\cdot \vec{u}$

\subsubsection{$x^*$ is a global-min of $f$ $\Leftrightarrow$ $t=0$ is the global-min of $\phi_{\vec{u}}(t)=f(\vec{x}+t\vec{u})$}
\begin{lemma}
