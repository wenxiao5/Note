...
\begin{equation}
    ...
    \nonumber
\end{equation}
$x^{(k)}\in\argmin_{x}\frac{1}{2}({x_1}^2+{x_2}^2)+\lambda({x_1}-1)+\frac{c_k}{2}({x_1}-1)^2$ $\Rightarrow$ $x^{(k)}=\left(\frac{c_k-\lambda^{(k)}}{c_k+1},0\right)$.
\begin{equation}
    \begin{aligned}
        \lambda^{(k+1)}&=\lambda^{(k)}+c_k \left(\frac{c_k-\lambda^{(k)}}{c_k+1}-1\right)=\frac{\lambda^{(k)}}{c_k+1}-\frac{c_k}{c_k+1}
    \end{aligned}
    \nonumber
\end{equation}
