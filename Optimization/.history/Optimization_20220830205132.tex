...
\begin{enumerate}
    ...
    \item \textbf{Second derivative:} $\vec{u}^T Hf(\vec{x}+t\vec{u})\vec{u}$ $$\phi^{''}_{\vec{u}} (t)=\sum_{i=1}^n \sum_{j=1}^n u_iu_j\frac{\partial^2 f}{\partial x_i\partial x_j}(\vec{x}+t\vec{u})$$
\end{enumerate}
