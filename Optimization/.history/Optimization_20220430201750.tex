...
If $x^*,\lambda^*$ satisfy the second-order sufficiency condition (for $x^*$ being a strict local min for $(P)$), then $\exists \bar{c}>0$ s.t. if $c\geq \bar{c}$, \underline{$x^*$ is a local min of $L_c(x,\lambda^*)$}
